...
class TaxiService
  ...
  def assignRider
    while @riderQueue.length > 0
      if @taxiQueue.length == 0
        p "I came here to send Taxies and pick up riders... and I'm all outta Taxies"
      break
      elsif @taxiQueue[0].pickup(@riderQueue[0])
        @riderQueue.shift()
      elsif
        @taxiQueue.shift()
      end
...
taxi0 = Taxi.new("Literally the guy from Taxi Driver", 4, "Checker", "A10", "TLKN2ME")
